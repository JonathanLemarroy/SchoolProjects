...
def joinFactors(factors):
    ...
    for factor in factors:
        for var in factor.unconditionedVariables():
            uncondVariables.add(var)
        for var in factor.conditionedVariables():
            condVariables.add(var)

    for uncondVar in uncondVariables:
        if uncondVar in condVariables:
            condVariables.remove(uncondVar)

    jointFactor = Factor(uncondVariables, condVariables, factors[0].variableDomainsDict())

    for assignment in jointFactor.getAllPossibleAssignmentDicts():
        probability = 1
        for factor in factors:
            probability *= factor.getProbability(assignment)
        jointFactor.setProbability(assignment, probability)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        newFactor = Factor(unConVars, factor.conditionedVariables(), factor.variableDomainsDict())
...
def normalize(factor):
    ...
    for var in unConVars:
        if len(domain[var]) == 1:
            conVars.add(var)
        if var not in conVars:
            temp.add(var)

    unConVars = temp
    newFactor = Factor(unConVars, conVars, domain)
    ...
    for assignment in assignments:
        newFactor.setProbability(assignment, factor.getProbability(assignment) / prob)
