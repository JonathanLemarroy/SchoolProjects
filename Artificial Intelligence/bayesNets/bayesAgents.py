...
def constructBayesNet(gameState):
    ...
    for h in gameState.getPossibleHouses():
        for w in gameState.getHouseWalls(h):
            obs = OBS_VAR_TEMPLATE % w
            obsVars.append(obs)
            edges.append((FOOD_HOUSE_VAR, obs))
            edges.append((GHOST_HOUSE_VAR, obs))
            variableDomainsDict[obs] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
...
def fillObsCPT(bayesNet, gameState):
    ...
    for h in gameState.getPossibleHouses():
        for w in gameState.getHouseWalls(h):
            obs = OBS_VAR_TEMPLATE % w
            obsFactor = bn.Factor([obs], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                pos = None
                prob = 0
                width, height = gameState.data.layout.width, gameState.data.layout.height
                
                if w[0] > width / 2.0 and w[1] > height / 2.0:
                    pos = TOP_RIGHT_VAL
                elif w[0] < width / 2.0 and w[1] > height / 2.0:
                    pos = TOP_LEFT_VAL
                elif w[0] < width / 2.0 and w[1] < height / 2.0:
                    pos = BOTTOM_LEFT_VAL
                elif w[0] > width / 2.0 and w[1] < height / 2.0:
                ...
                if assignment[FOOD_HOUSE_VAR] == pos:
                    if assignment[obs] == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                    elif assignment[obs] == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    ...
                    if assignment[obs] == BLUE_OBS_VAL:
                        ...
                        prob = PROB_GHOST_RED
                elif assignment[obs] == NO_OBS_VAL:
                        prob = 1
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        rightV.update(evidence)
        leftV.update(evidence)
        
        rightV[GHOST_HOUSE_VAR] = TOP_LEFT_VAL
        rightV[FOOD_HOUSE_VAR] = TOP_RIGHT_VAL
        leftV[FOOD_HOUSE_VAR] = TOP_LEFT_VAL
        leftV[GHOST_HOUSE_VAR] = TOP_RIGHT_VAL

        infr = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], evidence, eliminationOrder)

        leftProb = infr.getProbability(leftV)
        rightProb = infr.getProbability(rightV)

        leftExpectedValue = WON_GAME_REWARD * leftProb + GHOST_COLLISION_REWARD * rightProb 
        rightExpectedValue = GHOST_COLLISION_REWARD * leftProb + WON_GAME_REWARD * rightProb
