...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            tmpDist = self.getPositionDistribution(gameState, pos)
            for key in tmpDist.keys():
                newDistribution[key] += tmpDist[key] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        probDist = self.getBeliefDistribution()
        for pos in self.legalPositions:
            probDist[pos] *= self.getObservationProb(observation, gameState.getPacmanPosition(), pos, self.getJailPosition())
        if probDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            newParticles = []
            for i in range(self.numParticles):
                newParticles.append(probDist.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.legalPositions:
            tmpDist = self.getPositionDistribution(gameState, pos)
            for key in tmpDist.keys():
                newDistribution[key] += tmpDist[key] * oldDist[pos]
        newDistribution.normalize()
...
class JointParticleFilter(ParticleFilter):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        # "*** YOUR CODE HERE ***"
        probDist = DiscreteDistribution()
        for pos in self.particles:
            prob = 1
            for ghost in range(self.numGhosts):
                prob *= self.getObservationProb(observation[ghost], gameState.getPacmanPosition(), pos[ghost], self.getJailPosition(ghost))
            probDist[pos] += prob
        if probDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
