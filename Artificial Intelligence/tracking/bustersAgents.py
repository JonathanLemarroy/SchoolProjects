...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPosDist in livingGhostPositionDistributions:
            gostPositions.append(ghostPosDist.argMax())
        
        shortestDistance = float("inf")
        target = pacmanPosition
        
        for position in gostPositions:
            dist = self.distancer.getDistance(pacmanPosition, position)
            if shortestDistance != min(shortestDistance, dist):
                target = position
        ...
        for action in legal:
            ...
            dist = self.distancer.getDistance(successor, target)
            if shortestDistance != min(shortestDistance, dist):
                actionTaken = action
                shortestDistance = dist
